...
import csv
sys.path.append("..")		
from toolkit.pre_load import pre_load_thu,neo_con, url_name
...
def get_NE(text):
	...
	while i < length:# 0 1 2 3 4
		...
		if p12 in url_name and flag != None and preok(t1) and nowok(t2):  # 组合2个词如果得到实体  如果分词连接后的单词在label中, 且通过分词连接后的单词在数据库中,
		# 	#preok是上一个词词性筛选，nowok是当前此词向筛选
			answerList.append([p12,url_name[p12]])#如果条件都通过,则将label和搜索的存入
			i += 2
			continue#跳过,进行下次

		flag = db.matchHudongItembyTitle(p1)#未组合时的单词
		if p1 in url_name and flag != None and nowok(t1):	 # 当前词如果是实体
			answerList.append([p1,url_name[p1]])
			i += 1 #继续下一个未运用词
			continue#不在往下,知识while,

		# flag = db.matchHudongItembyTitle(p2)  # 未组合时的单词
		# if p1 in url_name and flag != None and nowok(t1):  # 当前词如果是实体
		# 	answerList.append([p1, url_name[p1]])
		# 	i += 1  # 继续下一个未运用词
		# 	continue  # 不在往下,知识while,

		if temporaryok(t1):#起始词向通过,则放入
			answerList.append([p1,t1])
			i += 1
			continue
			
		answerList.append([p1,0])#最后在+0
		i += 1
		
	return answerList#实体筛选
