...
def search_entity(request):
	...
	if(request.GET):
		...
		if len(entityRelation) == 0:
			...
			return render(request,'search_entity.html',{'entityRelation':json.dumps(entityRelation,ensure_ascii=False)})

	return render(request,"search_entity.html",{'ctx':ctx})
...
def search_relation(request):
	...
	if(request.GET):
		...
		if(len(entity1) !=0 and len(relation) == 0 and len(entity2)!=0): #entity1=苹果， relation=""  entity2=苹果  这是会报错的
